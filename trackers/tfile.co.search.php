<?php
include_once('tfile.co.engine.php');
	...
	public static function mainSearch($params)
	{
    	...
    	$page = Sys::getUrlContent(
        	array(
        		...
        		'header'         => 0,
        		'returntransfer' => 1,
        		'url'            => 'http://search.tfile.co/?q=&c=0&g=&act=&y=&ql=&a='.$user.'&d=&o=&size_min=0&size_max=0',
            ...
            for ($i=0; $i<count($toDownload); $i++)
            {
            	...
            	$page = Sys::getUrlContent(
	            	array(
	            		...
	            		'header'         => 0,
	            		'returntransfer' => 1,
	            		'url'            => 'http://tfile.co/forum/viewtopic.php?t='.$toDownload[$i]['threme_id']
	            	)
	            );
				...
				if (preg_match('/download\.php\?id=(\d+)&uk=(\d+)/', $page, $link))
				{
					...
					$torrent = Sys::getUrlContent(
                    	array(
                    		'type'           => 'GET',
                    		'returntransfer' => 1,
                    		'url'            => 'http://tfile.co/forum/download.php?id='.$download_id.'&ak='.$ak_id,
                    		'sendHeader'     => array('Host' => 'tfile.co'),
                    		'referer'        => 'http://tfile.co/forum/viewtopic.php?t='.$torrent_id,
                    	)
                    );
